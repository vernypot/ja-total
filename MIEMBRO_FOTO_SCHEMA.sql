-- =============================================================================
-- Member profile photos (miembros.foto_url + Supabase Storage)
-- Run in Supabase Dashboard → SQL Editor
-- Prerequisite: MIEMBRO_CLASE_PROGRESIVA_RLS_FIX.sql (user_can_manage_miembro)
-- If upload fails with RLS, also run MIEMBRO_FOTO_RLS_FIX.sql
-- =============================================================================

ALTER TABLE public.miembros
  ADD COLUMN IF NOT EXISTS foto_url TEXT;

COMMENT ON COLUMN public.miembros.foto_url IS 'Public URL of member profile photo in miembro-fotos bucket';

-- ---------------------------------------------------------------------------
-- Storage bucket (public read for img tags)
-- ---------------------------------------------------------------------------
...
-- Helpers for storage path checks
...
-- ---------------------------------------------------------------------------
-- RPC to update foto_url (bypasses miembros RLS)
-- ---------------------------------------------------------------------------
