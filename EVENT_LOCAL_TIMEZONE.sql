-- =============================================================================
-- Church-local timezone helpers for all event date/time validation
-- Run in Supabase Dashboard → SQL Editor (includes EVENTO_CHECKIN_TIMEZONE_FIX)
--
-- Event fecha + hora are wall-clock times in America/Bogota, not UTC.
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.church_local_today()
RETURNS DATE
LANGUAGE sql
STABLE
SET search_path = public
...
CREATE OR REPLACE FUNCTION public.evento_is_future(p_fecha DATE, p_hora TIME)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  SELECT public.evento_start_at(p_fecha, p_hora) > now();
$$;

CREATE OR REPLACE FUNCTION public.evento_is_past(p_fecha DATE, p_hora TIME)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  SELECT NOT public.evento_is_future(p_fecha, p_hora);
$$;

COMMENT ON FUNCTION public.evento_start_at(DATE, TIME) IS
  'Converts event fecha+hora (church wall clock) to timestamptz using America/Bogota.';

COMMENT ON FUNCTION public.church_local_today() IS
  'Current date in America/Bogota for event filtering and validation.';

-- Ensure check-in uses church-local start time (re-apply if already patched)
...
GRANT EXECUTE ON FUNCTION public.church_local_today() TO authenticated;
GRANT EXECUTE ON FUNCTION public.evento_is_future(DATE, TIME) TO authenticated;
GRANT EXECUTE ON FUNCTION public.evento_is_past(DATE, TIME) TO authenticated;
