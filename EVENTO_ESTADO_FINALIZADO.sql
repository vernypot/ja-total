-- =============================================================================
-- Eventos: add closing status "finalizado" (shown as "Ended" in the UI)
-- Run in Supabase Dashboard → SQL Editor after EVENTOS_SCHEMA.sql
-- Re-applies admin_checkin_evento with a guard for non-active events.
-- =============================================================================
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.eventos DROP CONSTRAINT eventos_estado_check;
  END IF;
END $$;
...
COMMENT ON COLUMN public.eventos.estado IS
  'activo = open; finalizado = ended/closed; inactivo = hidden; cancelado = cancelled';
