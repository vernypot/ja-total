...
-- Linked system user (dashboard login) confirming as their own member profile
DROP FUNCTION IF EXISTS public.usuario_set_linked_miembro_evento_confirmacion(UUID, TEXT, UUID);
DROP FUNCTION IF EXISTS public.usuario_set_linked_miembro_evento_confirmacion(TEXT, UUID, UUID);

CREATE OR REPLACE FUNCTION public.usuario_set_linked_miembro_evento_confirmacion(
  p_confirmacion_estado TEXT,
  p_evento_miembro_id UUID DEFAULT NULL,
  p_evento_id UUID DEFAULT NULL
)
...
GRANT EXECUTE ON FUNCTION public.usuario_set_linked_miembro_evento_confirmacion(TEXT, UUID, UUID)
  TO authenticated;
