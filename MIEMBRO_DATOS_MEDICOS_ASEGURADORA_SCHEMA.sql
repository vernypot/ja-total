-- =============================================================================
-- Medical data: aseguradora + Seguro Denominacional (checkbox + date)
...
ALTER TABLE public.miembro_datos_medicos
  ...
  ADD COLUMN IF NOT EXISTS seguro_denominacional BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS seguro_denominacional_fecha DATE;

-- Copy legacy EPS column into aseguradora when present
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'miembro_datos_medicos'
      AND column_name = 'eps'
  ) THEN
    UPDATE public.miembro_datos_medicos
    SET aseguradora = COALESCE(NULLIF(trim(aseguradora), ''), NULLIF(trim(eps), ''))
    WHERE aseguradora IS NULL OR trim(aseguradora) = '';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'miembro_datos_medicos_seguro_denominacional_fecha_check'
  ) THEN
    ALTER TABLE public.miembro_datos_medicos
      ADD CONSTRAINT miembro_datos_medicos_seguro_denominacional_fecha_check CHECK (
        seguro_denominacional = false OR seguro_denominacional_fecha IS NOT NULL
      );
  END IF;
END $$;

COMMENT ON COLUMN public.miembro_datos_medicos.aseguradora IS
...
COMMENT ON COLUMN public.miembro_datos_medicos.seguro_denominacional IS
  'Whether the member has denominational (church) insurance coverage.';
COMMENT ON COLUMN public.miembro_datos_medicos.seguro_denominacional_fecha IS
  'Date associated with denominational insurance (enrollment, renewal, etc.).';
