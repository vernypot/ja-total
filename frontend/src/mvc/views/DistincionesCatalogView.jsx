...
export default function DistincionesCatalogView({
  items,
  showInactive,
  setShowInactive,
  showForm,
  ...
  return (
    <div className="container">
      <div className="page-header">
        <div>
          <h1>🏅 {t('distinciones')} <PageHelpLink pageId="distinciones" /></h1>
        ...
        {canManage && (
          ...
          >
            {showForm ? `✕ ${t('cancel')}` : `➕ ${t('newDistincion')}`}
          </button>
        )}
      </div>
      ...
      {!hasTable && (
        <div className="alert alert-error">{t('distincionesSchemaMissing')}</div>
      )}
      ...
      <div className="card">
        ...
        {showForm && canManage && (
          <div style={{ padding: '15px', backgroundColor: '#f0f9ff', border: '2px solid #0891b2', borderRadius: '8px', marginBottom: '20px' }}>
            <h4 style={{ marginTop: 0 }}>{editingId ? t('editDistincion') : t('newDistincion')}</h4>
        ...
        {loading ? (
          ...
          <p className="text-muted">{t('noDistinciones')}</p>
        ) : (
