    ...
    <div className="container">
      ...
      <div className="card" style={{ marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
          <input type="text" placeholder="Search labels..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="form-input" style={{ flex: 1, minWidth: '200px' }} />
        ...
        {formMode === 'add' && (
          <div style={{ padding: '20px', backgroundColor: '#f0f9ff', border: '2px solid #0891b2', borderRadius: '8px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px' }}>
              {['label_key', 'label_es', 'label_en'].map(field => (
                <div key={field}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
              <button onClick={addLabel} style={{ padding: '10px 20px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}>✓ Save</button>
              <button onClick={() => { setFormMode('view'); setNewLabel({ label_key: '', label_es: '', label_en: '' }); }} style={{ padding: '10px 20px', backgroundColor: '#6b7280', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}>✕ Cancel</button>
            </div>
          </div>
        )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead>
                <tr style={{ backgroundColor: '#f3f4f6', borderBottom: '2px solid #e5e7eb' }}>
                  {['Key', 'Spanish', 'English', 'Actions'].map(h => (
                    <th key={h} style={{ padding: '12px', textAlign: h === 'Actions' ? 'center' : 'left', fontWeight: 'bold' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
